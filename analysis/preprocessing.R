...
for (vp in vps){
  ...
  v_df <- trial_df[trial_df$subjectID == vp, ]
    
  # ... erstellen wir einen leeren plot
  plot(NULL, xlim=c(250, 1250), ylim = c(440,510), ylab = "vertikale Position", 
...
for (vp in vps){
  ...
  # Leerer Plot
  plot(NULL, xlim=c(-700, 700), ylim = c(-50,50), ylab = "vertikale Position", 
       xlab = "horizontale Position", main = vp)
...
minimal_df <- trial_df[c('trialID', 'subjectID','angle', 'side',
                         'velx0_99', 'vely0_99', 
